...
const TierTable = () => {
  ...
  // S+: 100 약상, 130 실돈키, 119, 적슈 128 돌로쟈,  118 마히스
  const S = [100, 130, 119, 128, 118];

  // S:  120 죽나상, 13 후파우, 122 츠이스, 72 쥐싱, 61 디로쟈,98 넬슈, 85 디루, 132 탕히스,  27 퀴히스, 131 탕히스 75 섕싱, 121 제로쟈, 123 섕르소, 129 제싱클, 103 런싱, 137 리상
  const A = [
    120, 13, 122, 72, 61, 98, 85, 132, 27, 131, 75, 121, 123, 129, 103, 137,
  ];

  // A: 136 흑이스, 135 흑히스, 10 쥐파, 125 혈티스, 3 떱돈, 66 떱슈, 23 r히스, 114 디뫼, 96 퐁그렉, 20 갈그렉, 32 k루, 30 콩루, 83 마티스, 94 치티스, 102 선장마엘
  const B = [136, 135, 10, 125, 3, 66, 23, 114, 96, 20, 32, 30, 83, 94, 102];

  // B: 81 떱상, 115 떱티스, 6 중돈,  79 동상 , 127 혈그렉
  const C = [81, 115, 6, 79, 127];

  // C: 134 홍파우, 91 리쟈, 40 리우마엘, 12 세파우
  const D = [134, 91, 40, 12];
